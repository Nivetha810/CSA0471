...
// Create an index block that contains pointers to data blocksint indexBlock[100] = {0}; // Adjust the size as needed

// Open or create a file in write mode (for writing blocks)file =
 
fopen("indexed_file.txt", "w"); if (file == NULL) {
...
{
printf("Block Number: "); scanf("%d", &block.blockNumber);if
(block.blockNumber == 0) { break;
}
