#include <stdio.h>
#define MAX_FRAMES 3 // Maximum number of frames in memory void printFrames(int frames[], int n) {for
(int i = 0; i < n; i++) { if (frames[i] == -1) {
printf(" - ");
} else {
printf(" %d ", frames[i]);
}
...
sizeof(referenceString) / sizeof(referenceString[0]); int frames[MAX_FRAMES];
